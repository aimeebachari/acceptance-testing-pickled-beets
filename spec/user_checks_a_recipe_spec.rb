...
feature "User checks a recipe's deliciousness", %(
  ...
  scenario "user submits a recipe name containing 'pickled beets'" do
    visit "/"
    fill_in "Recipe Name", with: "stinky pickled beets"
    click_button "Submit"

    expect(page).to have_content("\"stinky pickled beets\" is a delicious recipe! Try again!")
  end

  scenario "user submits a recipe name without 'pickled beets'" do
    visit "/"
    fill_in "Recipe Name", with: "spaghetti"
    click_button "Submit"

    expect(page).to have_content("\"spaghetti\" is not a delicious recipe! Try again!")
  end

  scenario "user navigates back to the home page after checking a recipe name" do
    visit "/"
    fill_in "Recipe Name", with: "yummy beets"
    click_button "Submit"
